...
class HasToReal (R : Type uR) : Type (max uR 1) where
  toRealRingHom : R →+* ℝ

namespace HasToReal

@[simp] lemma toRealRingHom_apply [HasToReal R] (x : R) :
    (HasToReal.toRealRingHom (R := R)) x = HasToReal.toRealRingHom (R := R) x := rfl
...
-/
class HasToRealOrder (R : Type uR) [LE R] extends HasToReal R : Prop where
  mono_toReal : ∀ {a b : R}, a ≤ b → toRealRingHom a ≤ toRealRingHom b

namespace HasToRealOrder

variable [LE R] [HasToRealOrder R]
...
-/
class IsHamiltonianR (NN : NeuralNetwork R U σ) [LE R] where
...
variable (NN : NeuralNetwork R U σ) [LE R] [HasToRealOrder R] [Fintype NN.State] [DecidableEq U]
...
lemma energyToReal_is_lyapunov
    (p : Params NN) (s : NN.State) (u : U) :
    energyToReal (NN := NN) p (NeuralNetwork.State.Up s p u) ≤ energyToReal (NN := NN) p s := by
  -- transport the scalar inequality through monotonicity of `toReal`
  dsimp [energyToReal]
...
variable (NN : NeuralNetwork R U σ) [LE R] [HasToRealOrder R]
variable [Fintype NN.State] [DecidableEq U]
variable [IsHamiltonianR (R := R) (U := U) (σ := σ) NN]
