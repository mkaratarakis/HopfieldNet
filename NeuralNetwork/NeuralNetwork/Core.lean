...
-/
def θ' (θ : Vector R 1) : R := θ.get ⟨0, by decide⟩
...
variable {u : U} (wθ : Params (HopfieldNetwork R U))
...
@[simp]
lemma act_of_non_up {v2 : U} (huv : v2 ≠ u) : (s.Up wθ u).act v2 = s.act v2 := by
  simp [Up, huv]
...
@[simp]
lemma act_new_neg_one_if_not_net_lt_th (hn : ¬s.net wθ u < θ' (wθ.θ u)) :
    (s.Up wθ u).act u = 1 := by
...
lemma act_new_neg_one_if_net_eq_th (hn : s.net wθ u = θ' (wθ.θ u)) : (s.Up wθ u).act u = 1 := by
  rw [act_up_def]; exact ite_eq_left_iff.mpr fun hyp =>
    (hyp (le_iff_lt_or_eq.mpr (Or.inr hn.symm))).elim

@[simp]
lemma activ_old_one (hc : (s.Up wθ u).act u ≠ s.act u) (hn : s.net wθ u < θ' (wθ.θ u)) :
    s.act u = 1 :=
...
lemma Ew_update_formula_split : s.Ew wθ = (- ∑ v2 ∈ {v2 | v2 ≠ u}, s.Wact wθ v2 u) +
  ...
  · rw [mul_add, ← add_assoc, add_right_cancel_iff]
    ...
    have sum_v1_v2_not_eq_v1_eq_u :
        ∑ v1, (∑ v2 ∈ {v2 | v2 ≠ v1 ∧ v1 = u}, s.Wact wθ v1 v2) =
        ∑ v2 ∈ {v2 | v2 ≠ u}, s.Wact wθ u v2 := by
    ...
    have sum_v1_v2_not_eq_v1_eq_u' :
    ∑ v1, (∑ v2 ∈ {v2 | (v2 ≠ v1 ∧ v1 ≠ u) ∧ v2 = u}, s.Wact wθ v1 v2) = ∑ v1 ∈ {v1 | v1 ≠ u},
        s.Wact wθ u v1 := by
      rw [← Ew_sum_formula_eq]; nth_rw 2 [sum_over_subset]; rw [sum_congr rfl]; intro v1 hv1
      have sum_Wact_v1_u : ∑ v2 ∈ {v2 | v2 ≠ v1 ∧ v2 = u}, s.Wact wθ v1 v2 =
          if v1 ≠ u then s.Wact wθ v1 u else 0 := by
        split
    ...
    have sum_Wact_eq_sum_Wact_sym : ∑ v1, ∑ v2 ∈ {v2 | v2 ≠ v1 ∧ v2 = u}, s.Wact wθ v1 v2 =
      ...
      rw [Ew_sum_formula_eq, sum_v1_v2_not_eq_v1_eq_u'];
      apply sum_congr rfl (fun _ _ => Wact_sym wθ u _)
...
lemma Ew_diff' : (s.Up wθ u).Ew wθ - s.Ew wθ =
    - ∑ v2 ∈ {v2 | v2 ≠ u}, (s.Up wθ u).Wact wθ v2 u -
      (- ∑ v2 ∈ {v2 | v2 ≠ u}, s.Wact wθ v2 u) := by
  ...
  calc _ = -∑ v2 ∈ {v2 | v2 ≠ u}, (s.Up wθ u).Wact wθ v2 u +
       ...
       _ = - ∑ v2 ∈ {v2 | v2 ≠ u}, (s.Up wθ u).Wact wθ v2 u -
        (- ∑ v2 ∈ {v2 | v2 ≠ u}, s.Wact wθ v2 u) := ?_
...
theorem Eθ_diff : (s.Up wθ u).Eθ wθ - s.Eθ wθ = θ' (wθ.θ u) * ((s.Up wθ u).act u - s.act u) := by
  calc _ =  θ' (wθ.θ u) * (s.Up wθ u).act u + ∑ v2 ∈ {v2 | v2 ≠ u}, θ' (wθ.θ v2) *
            (s.Up wθ u).act v2 +
...
lemma E_final_Form : (s.Up wθ u).E wθ - s.E wθ = (s.act u - (s.Up wθ u).act u) *
  ...
  calc _ = (s.Up wθ u).Eθ wθ- s.Eθ wθ +  (s.Up wθ u).Ew wθ - s.Ew wθ := ?_
       ...
       _ = ∑ v2 ∈ {v2 | v2 ≠ u}, (- wθ.w v2 u * s.act v2 *
          (s.Up wθ u).act u + wθ.w v2 u * s.act v2 * s.act u)
       ...
       _ = ((s.Up wθ u).act u - s.act u) * ∑ v2 ∈ {v2 | v2 ≠ u}, - (wθ.w v2 u * s.act v2)
          + θ' (wθ.θ u) * ((s.Up wθ u).act u - s.act u) := ?_
       _ = ((s.Up wθ u).act u - s.act u) * (∑ v2 ∈ {v2 | v2 ≠ u},
          - (wθ.w v2 u * s.act v2) + θ' (wθ.θ u)) := ?_
       _ =  ((s.Up wθ u).act u - s.act u) * - (∑ v2 ∈ {v2 | v2 ≠ u},
          (wθ.w v2 u * s.act v2) + - θ' (wθ.θ u)) := ?_
       _ = - ((s.Up wθ u).act u - s.act u) * ((∑ v2 ∈ {v2 | v2 ≠ u},
          wθ.w v2 u * s.act v2) - θ' (wθ.θ u)) := ?_
       _ = (s.act u - (s.Up wθ u).act u) * ((∑ v2 ∈ {v2 | v2 ≠ u},
          wθ.w u v2 * s.act v2) - θ' (wθ.θ u)) := ?_
...
instance : Fintype ((HopfieldNetwork R U).State) :=
  Fintype.ofEquiv _ ((stateToNeurActMap_equiv').symm)
...
def seqStates' {wθ : Params (HopfieldNetwork R U)} (s : State' wθ) (useq : ℕ → U) :
    ℕ → State' wθ :=
  seqStates wθ s useq
...
-/
def stateLt (s1 s2 : State' wθ) : Prop := s1.E wθ < s2.E wθ ∨ s1.E wθ =
    s2.E wθ ∧ s2.pluses < s1.pluses
...
@[simp]
lemma n_leq_n'_imp_sseq_n (n : ℕ) (useq : ℕ → U) :
...
@[simp]
lemma n_leq_n'_imp_sseq_n_k'' (n : ℕ) (useq : ℕ → U) :
  (seqStates wθ s useq (n+1)) = (seqStates wθ s useq n).Up wθ (useq n):= rfl

@[simp]
lemma n_leq_n'_imp_sseq_n_k (n k : ℕ) (useq : ℕ → U) :
  (seqStates wθ s useq ((n + k) + 1)) = (seqStates wθ s useq (n + k)).Up wθ (useq (n + k)) := by
  simp only [seqStates]

@[simp]
lemma NeuralNetwork.n_leq_n'_imp_sseq_n'_leq_sseq'' (s : State' wθ) (n k : ℕ) (useq : ℕ → U) :
  seqStates' s useq (n + k) ≤ seqStates' s useq n := by
  induction k with
  | zero => simp only [Nat.add_zero]; apply le_refl
  | succ k hk =>
      rw [Nat.add_succ, seqStates', n_leq_n'_imp_sseq_n_k]; trans;
      apply update_le; exact hk
...
theorem seqStates_lt (s : State' wθ) (useq : ℕ → U) (n : ℕ) (m' : ℕ) (hm' : m' > n) :
  ...
  exact n_leq_n'_imp_sseq_n'_leq_sseq'' s n (k + 1) useq
...
lemma not_stable_implies_sseqm_lt_sseqn_cyclic (useq : ℕ → U) (hf : cyclic useq) (n : ℕ)
  ...
  have : (Up' (seqStates' s useq n) u).act u = (seqStates' s useq n).act u ↔
      (Up' (seqStates' s useq n) u) = (seqStates' s useq n) :=
        up_act_eq_iff_eq (seqStates' s useq n)
  rw [this] at hc
...
/--
`HopfieldNet_conv_time_steps` calculates the number of time steps required
for a Hopfield Network to converge.
-/
  ...
  have : ∀ i j, (dotProduct (ps i).act (ps j).act) = if i ≠ j then 0 else (card U : R) := by
    ...
    · simp only [Decidable.not_not] at h
      ...
      · unfold dotProduct
        ...
        have hact1 : ∀ i, ((ps j).act i) * ((ps j).act i) = 1 :=
          fun i => mul_self_eq_one_iff.mpr (hact i)
