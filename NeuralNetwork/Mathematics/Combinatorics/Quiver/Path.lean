...
def lift_path_to_induced {S : Set V} [DecidablePred (· ∈ S)]
    ...
    letI : Quiver S := Quiver.inducedQuiver S
...
lemma remove_cycle_gives_shorter_path [DecidableEq V] {a v b : V}
  ...
  have hlt :
      p_prefix.length + p_rest.length <
      p_prefix.length + p_rest.length + p_cycle.length :=
        Nat.lt_add_of_pos_right h_cycle_pos
  simpa [length_comp, Nat.add_comm, Nat.add_left_comm, Nat.add_assoc] using hlt
...
lemma length_le_card_minus_one_of_isSimple [Fintype V] [DecidableEq V] [Quiver V] {a b : V} (p : Path a b) (hp : p.IsStrictlySimple) :
  ...
  have h_card_le_univ : p.vertexFinset.card ≤ Fintype.card V := by
