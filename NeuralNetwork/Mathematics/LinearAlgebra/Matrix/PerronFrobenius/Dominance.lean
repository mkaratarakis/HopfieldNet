...
open Quiver
open Matrix Complex Classical

variable (n) {A : Matrix n n ℝ}
...
theorem mul_mulVec {α : Type*} {l m n : Type*}
    [NonUnitalSemiring α] [Fintype m] [Fintype l] [Fintype n]
...
lemma eigenvector_norm_pos_of_primitive_and_norm_eq_perron_root
  ...
  have h_r_pos : 0 < perronRoot_alt A := by
    obtain ⟨r', v, hr'_pos, hv_pos, h_eig'⟩ := exists_positive_eigenvector_of_primitive _ hA_prim hA_nonneg
    have : r' = perronRoot_alt A := by
      ...
      · exact h_eig'
    rwa [← this]
  exact eigenvector_of_primitive_is_positive _ hA_prim h_r_pos h_x_abs_eig h_x_abs_nonneg h_x_abs_ne_zero
