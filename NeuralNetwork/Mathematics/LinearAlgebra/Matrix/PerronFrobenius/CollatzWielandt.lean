...
theorem exists_maximizer (A : Matrix n n ℝ) [DecidableEq n] :
...
lemma eq_iInf_of_nonempty
  ...
  · intro i
    ...
    apply Subtype.mk
    · exact property
  · simp only [Subtype.exists, toFinset_setOf, mem_filter, Finset.mem_univ, true_and]
...
lemma exists_mulVec_eq_zero_on_support_of_nonpos [Fintype n]
  ...
  have h_vb_ne : v b ≠ 0 := ne_of_gt h_vb_pos
  have h_num_zero : (A *ᵥ v) b = 0 := by
    have h' := congrArg (fun t => t * v b) h_ratio_zero
    grind
  exact ⟨b, hb_mem, h_num_zero⟩
