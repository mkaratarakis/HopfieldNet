...
/-!
## Key lemmas for Compactness & Ultrafilters
-/

/- Ultrafilter existence lemma
lemma Ultrafilter.exists_le {α : Type*} {F : Filter α} (h : F.NeBot) :
  ∃ U : Ultrafilter α, (U : Filter α) ≤ F := by
  exact Ultrafilter.exists_le F

-- Compactness characterization via ultrafilters
lemma isCompact_iff_ultrafilter_le_nhds {X : Type*} [TopologicalSpace X] {s : Set X} :
...
lemma IsCompact.exists_clusterPt {X : Type*} [TopologicalSpace X] {s : Set X}
...
lemma isCompact_stdSimplex (ι : Type*) [Fintype ι] : IsCompact (stdSimplex ℝ ι) := by
  exact _root_.isCompact_stdSimplex ι

-- Standard simplex is convex
lemma convex_stdSimplex (𝕜 ι : Type*) [OrderedRing 𝕜] [Fintype ι] :
...
lemma IsCompact.exists_max {s : Set ℝ} (hs : IsCompact s) (hne : s.Nonempty) :
  ∃ x ∈ s, ∀ y ∈ s, y ≤ x := by
  let sup_s := sSup s
...
lemma isMaxOn_eq_sSup {X : Type*} [TopologicalSpace X]
    {f : X → ℝ} {s : Set X} {v : X}
...
lemma Ultrafilter.eq_of_le {α : Type*} {u v : Ultrafilter α} (h : (u : Filter α) ≤ v) :
    u = v := by
  exact Ultrafilter.eq_of_le h

-- Tendsto characterization for ultrafilters
lemma tendsto_map'_iff {α β : Type*} {f : α → β} {u : Ultrafilter α} {l : Filter β} :
...
lemma eventually_to_open {α : Type*} [TopologicalSpace α] {p : α → Prop} {a : α}
    (h : ∀ᶠ x in 𝓝 a, p x) :
...
lemma continuousOn_finset_inf' {α β : Type*} [TopologicalSpace α] [LinearOrder β]
...
lemma finset_inf'_mono_subset {α β : Type*} [LinearOrder β] {s t : Finset α} (h : s ⊆ t)
...
lemma matrix_mulVec_component {n : Type*} [Fintype n] [DecidableEq n]
    (A : Matrix n n ℝ) (v : n → ℝ) (j : n) :
    (A *ᵥ v) j = ∑ i, A j i * v i := by
  simp [Matrix.mulVec]; rfl

-- Non-negative matrix preserves non-negative vectors
lemma mulVec_nonneg {n : Type*} [Fintype n] {A : Matrix n n ℝ} (hA : ∀ i j, 0 ≤ A i j)
...
lemma positive_mul_vec_pos {n : Type*} [Fintype n] [Nonempty n] {A : Matrix n n ℝ} (hA_pos : ∀ i j, 0 < A i j)
...
lemma exists_pos_of_sum_one_of_nonneg {n : Type*} [Fintype n] [Nonempty n] {x : n → ℝ}
...
lemma exists_ne_zero_of_ne_zero {n : Type*} [Fintype n] [Nonempty n] {x : n → ℝ} (hx : x ≠ 0) : ∃ j, x j ≠ 0 := by
  by_contra h
  push_neg at h
...
lemma pow_mulVec_succ {n : Type*} [Fintype n] [Nonempty n] [DecidableEq n] {A : Matrix n n ℝ} (k : ℕ) (x : n → ℝ) :
...
lemma iInf_apply_eq_finset_inf'_apply_fun {α β γ : Type*} [Fintype α] [Nonempty α]
...
lemma iInf_eq_ciInf {α β : Type*} [Fintype α] [Nonempty α] [ConditionallyCompleteLinearOrder β]
...
lemma mul_div_cancel_pos_right {K : Type*} [Field K] [LinearOrder K] [IsStrictOrderedRing K]
    {a b c : K} (h : a * b = c) (hb : 0 < b) : c / b = a := by
  rw [← h]
  exact mul_div_cancel_right₀ a hb.ne'

-- Non-positive times positive is non-positive
lemma mul_nonpos_of_nonpos_of_pos {α : Type*} [Ring α] [LinearOrder α] [IsStrictOrderedRing α]
...
lemma continuousOn_iInf {α β : Type*} [Fintype α] [Nonempty α] [TopologicalSpace β]
...
lemma sum_add_sum_erase {n : Type*} {M : Type*} [AddCommMonoid M] [DecidableEq n] {s : Finset n} {f : n → M}
    (a : n) (ha : a ∈ s) :
...
lemma Finset.disjoint_compl_right {n : Type*} [Fintype n] [DecidableEq n] {s : Finset n} :
  ...
  intro a haS haCompl
  rcases Finset.mem_sdiff.mp haCompl with ⟨_, ha_not_s⟩
  exact ha_not_s haS
...
lemma ratio_nonneg [Fintype n] (A : Matrix n n ℝ)
...
/-- If y is an upper bound of a set s, and x is in s, the x ≤ y -/
...
lemma mulVec_eq_zero_of_ratio_zero [Fintype n] (A : Matrix n n ℝ) {v : n → ℝ}
    (i : n) (hv_pos : 0 < v i)
...
lemma zero_block_of_mulVec_eq_zero [Fintype n] (A : Matrix n n ℝ)
    (hA_nonneg : ∀ i j, 0 ≤ A i j) {v : n → ℝ} (hv_nonneg : ∀ i, 0 ≤ v i)
...
lemma div_lt_iff_of_pos {a b c : ℝ} (hc : 0 < c) : b / c < a ↔ b < a * c := by
  have hc0 : c ≠ 0 := ne_of_gt hc
  constructor
  · intro h
    have := mul_lt_mul_of_pos_right h hc
    simpa [div_eq_mul_inv, hc0, mul_comm, mul_left_comm, mul_assoc] using this
  · intro h
    have := mul_lt_mul_of_pos_right h (inv_pos.mpr hc)
    simp_all only [ne_eq, not_false_eq_true, mul_inv_cancel_right₀, gt_iff_lt]
    exact this
...
lemma iSup_eq_sSup_image [ConditionallyCompleteLattice α] :
    (⨆ x : s, f x) = sSup (f '' s) := by
